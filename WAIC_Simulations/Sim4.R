...
run_sim4 <- function(iter){
  ...
  dat <- generate_data_TI(I_A, I_B, basis_coef_A, basis_coef_B, sigma_A, sigma_B, delta, 25, 25, n_AB_comp, 1, 1, 3, c(0,1), c(0.25, 0.5, 0.75))
...
df[(length(files) + 1):(2*length(files)),3] <- prop
df <- as.data.frame(df)
colnames(df) <- c("WAIC", "Method", "Prop_comp")
df$WAIC <- as.numeric(df$WAIC)
df$Method <- as.factor(df$Method)
df$Prop_comp <- as.numeric(df$Prop_comp)
ggplot(df, aes(x=Prop_comp, y=WAIC, colour = Method)) + scale_y_continuous(breaks = c(-900, -625, -400, -225, -100, -25, 0, 25, 100, 225, 400, 625, 900), trans = ssqrt_trans) + ylab(TeX("WAIC$_{comp}$ - WAIC$_{IIGPP}$"))+
  ...
  scale_colour_manual(values = c("WAIC Comp Conditional" = "#F8766D", "WAIC Comp Marginal" = "#00BA38")) + geom_smooth(method = "lm", se = F) + geom_hline(yintercept = 0,  color="red", linetype="dashed")
